## 2024-07-28 - Modal Keyboard Accessibility
...
## 2024-07-28 - Empty State ARIA Live Regions
...
## 2024-07-28 - Terminal Custom Inputs Need Focus/ARIA
**Learning:** Custom styled inputs (like the "terminal" design pattern with `outline-none`) look cool but default to having no visible focus state and zero accessibility context for form errors, heavily degrading the experience for keyboard navigators and screen reader users.
**Action:** Always manually re-add focus rings using `focus-visible:ring-1 focus-visible:ring-green-500 rounded` to custom inputs. Ensure validation errors are directly linked to their corresponding inputs using `aria-invalid`, `aria-describedby` pointing to the error message `id`, and adding `role="alert"` to the error text.
