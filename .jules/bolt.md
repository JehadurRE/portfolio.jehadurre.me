...
## 2024-11-20 - Prevent Date Instantiation Overhead in Sort Functions
**Learning:** Instantiating `new Date(a.date).getTime() - new Date(b.date).getTime()` inside a `.sort()` comparator creates an $O(N \log N)$ bottleneck because the JavaScript engine has to allocate new `Date` objects and parse date strings for every single comparison step. When sorting thousands of items (like blog posts or GitHub activity days), this causes significant main-thread blocking and frame drops.
**Action:** When sorting arrays by ISO 8601 date strings (like `YYYY-MM-DD` or full ISO timestamps), always replace Date object parsing with direct string comparisons (e.g., `a.date > b.date ? 1 : a.date < b.date ? -1 : 0`). ISO 8601 strings sort chronologically by default using standard string comparison, which executes instantly without any memory allocation overhead.
