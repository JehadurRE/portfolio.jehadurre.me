...
## 2024-11-20 - Optimizing Date Sorting in Render/Effect Loops
**Learning:** Instantiating `new Date(string)` inside `.sort()` array comparators (e.g., `new Date(a.date).getTime() - new Date(b.date).getTime()`) forces O(N log N) object allocations and parsing operations. For ISO 8601 formatted date strings, this is unnecessary overhead as they sort lexicographically by default.
**Action:** When sorting arrays by ISO 8601 date strings, do not instantiate new `Date` objects inside the `.sort()` comparator. Instead, use direct string comparisons (e.g., `a.date > b.date ? 1 : a.date < b.date ? -1 : 0`) to achieve the same chronological order while avoiding parsing and memory allocation overhead.
